...
정렬 모델 α를 피드포워드 신경망으로 매개변수화합니다. 이 신경망은 제안된 시스템의 다른 구성 요소들과 함께 공동으로 훈련됨.

전통적인 기계 번역과는 달리, 여기서는 정렬이 숨겨진 변수로 간주되지 않음. 대신에 정렬 모델은 직접적으로 소프트 정렬(soft alignment)을 계산. 이는 비용 함수의 그래디언트를 역전파할 수 있게 하며, 이 그라디언트는 정렬 모델과 전체 번역 모델을 함께 학습하는 데 사용될 수 있음.
